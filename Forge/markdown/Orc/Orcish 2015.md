...
This can create an ambiguity, because `Fu` and `Br` might both be words. We may wish to call `Fu` without the optional argument, then call `Br` immediately after. The `,` token resolves this: the recognizer simply passes on it, while it is guaranteed not to be a valid optional argument either. Thus `Fu , Br` provides `,` as the optional argument to `Fu`, which can leave it for the next parse, or consume it, with the same effect: we reset and call `Br`. It is good practice to use `,` when omitting optional arguments. 
